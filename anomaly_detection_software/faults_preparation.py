...
def visualize_error(correct_data, faulty_data, error_name):
    plt.plot(np.linspace(0, len(correct_data), len(correct_data)), correct_data, label='dane poprawne', color='green')
    plt.plot(np.linspace(0, len(faulty_data), len(faulty_data)), faulty_data, label='dane zakłócone', color='red')
    plt.xlabel("Dyskretne chwile czasu")
    plt.ylabel("Wartość sygnału mierzonego")
...
def insert_fault_erratic(df, faulty_params, intensity, time, period_length, period_diff=0.5, same_periods=False):
    ...
    for i in range(len(faulty_params)):
        ...
        errors = np.random.uniform(-intensity, intensity, size=(len(dat[0]),)) * mean
    ...
    # visualize_error(df_start.iloc[:, i].values, df.iloc[i:, ].values, "erratic")
    return df


def insert_fault_hardover(df, faulty_params, time, period_length, period_diff=0.5, same_periods=False):
    ...
    for i in range(len(faulty_params)):
        ...
        max_val = df[str(faulty_params[i]).split('_')[-1]].max()
        errors = np.repeat(1*max_val, len(dat[0]))
        errors = np.array([errors])
        dat = errors
